...
class transition_net():
    ...
    def save(self):
        ...
        try:
            self.model.save(os.path.join(self.path,"transition.h5"))
        except:
...
def cross_validation(k, max_entries):
    ...
    for test_id, train_id in kf.split(X):
        ...
        tn = transition_net(X.shape[1], Y.shape[1])
        ...
        score = tn.model.evaluate(X_test, y_test, verbose=0)
        mean += score[1]
...
if __name__ == '__main__':
   ...
   # data
    ...
    if (flags['cv']):
        ...
        tn = transition_net(X.shape[1], Y.shape[1], flags['agent_class'])
        tn.load()
        tn.fit(X,Y,
                epochs = flags['epochs'],
                batch_size = flags['batch_size'],
