...
class state_translator:
    ...
    def __init__(self, givenStateVector, givenPlayerCount):
        ...
        # a single bit indicating if a player has a handsize below max
        self.playerMissingCards = []
    ...
    def encodeVector(self):
        self.stateVector = self.handSpace + self.playerMissingCards + self.currentDeck + self.boardSpace + self.infoTokens + self.lifeTokens + self.discardSpace + self.lastActivePlayer + \
            self.lastMoveType + self.lastMoveTarget + self.colorRevealed + self.rankRevealed + \
            self.cardRevealed + self.positionPlayed + \
            self.cardPlayed + self.prevPlay + self.cardKnowledge
        if (len(self.stateVector) != self.stateVectorSize):
            raise ValueError(
                'stateVector size has changed since last encodeVector() call.')
    ...
    def decodeVector(self, stateVector):
        ...
        deckSize = 50 - (self.playerCount * handSize)
        ...
        self.cardKnowledge = stateVector[prevIndex:(
            prevIndex + self.playerCount * handSize * 35)]
