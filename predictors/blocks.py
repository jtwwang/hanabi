from tensorflow.keras.layers import Conv1D, BatchNormalization, Activation,\
    Dense, MaxPooling1D, UpSampling1D, Add, concatenate
...
def deconv_block(inputs, filters, kernel_size, strides, pool_size):
    """
    args:
        inputs: a tensor of shape (input_dim, 1)
        filters(int): how many filters in that layer
...
def dense_res_block(inputs, units):
    """
    args:
        inputs: a tensor
        units: the number of neurons in each dense layer
    """
    x = Dense(units)(inputs)
    x = Dense(units)(x)
    y = concatenate([inputs, x])
    return y
